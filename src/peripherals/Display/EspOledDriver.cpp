...
int EspOledDriver::writeCmd(uint8_t cmd) {
    uint8_t buf[1] = {cmd};
    return i2c->writeRegister(buf, OLED_I2C_ADDRESS, 0x00, 1);
}

int EspOledDriver::writeData(uint8_t *data, uint16_t size) {
    return i2c->writeRegister(data, OLED_I2C_ADDRESS, 0x40, size);
}

EspOledDriver::~EspOledDriver() {
}
